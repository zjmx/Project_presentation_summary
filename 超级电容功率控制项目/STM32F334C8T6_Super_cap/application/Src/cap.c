...
void mode_choose()
{
	...
	if(fabs(adc_data.cap_v-power_data.cap_max_change_V)>0.9f//电容电压与设定满电电压误差在1V左右判断为满电，后续修改为电荷量计算
		&&fabs(adc_data.chas_i)<0.1f)
	{
		power_data.power_mode=Buck;
	}
	//电容满电后开启Boost模式，准备输出
	else if(adc_data.cap_v>10.0f
				 &&fabs(adc_data.chas_i)>0.5f)
	{
...
{
	can_rx_data->power_mode=data[0]<<8;
