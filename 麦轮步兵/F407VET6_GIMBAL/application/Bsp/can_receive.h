...
typedef struct{
	...
	float rad_ecd;//弧度制电机角度
	
	pid_type_def gimbal_current_pid;//云台电机电流环PID数据指针
	pid_type_def gimbal_angle_pid;//云台电机角度环PID数据指针
	
	pid_type_def gimbal_mcu_current_pid;//云台电机电流环PID数据指针
	pid_type_def gimbal_mcu_angle_pid;//云台电机角度环PID数据指针
}motor_measure_t;
extern motor_measure_t motor_chassis[4];//rm电机数据返回结构体
extern motor_measure_t motor_gimbal[2];//rm电机数据返回结构体
